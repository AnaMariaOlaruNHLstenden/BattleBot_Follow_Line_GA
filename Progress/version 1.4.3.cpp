...
//Optimize the Avoid object and explain the code incease the labels for sections
}
    ...
    if (distance <= maxDistance && lineCount >= 4)
      {
        Serial.println("Avoid Object"); //This function makes sure that anything closer than 20CM it will avoid it
         ...
         Serial.println("left");
          ...
          fullScan();
        if (lineValues[3] >= MIN_BLACK || lineValues[4] >= MIN_BLACK || lineValues[2] >= MIN_BLACK || lineValues[5] >= MIN_BLACK) // This is to search for the line in case the turn needs to be longer
        {
          defaultLineSensor();
        }
        else
...
void testLights() {  // Set the color of the neopixels by pixel number and RGB value
  // Pixel number, Red, Green, Blue, Yellow
