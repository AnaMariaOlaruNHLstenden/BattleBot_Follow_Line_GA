...
//-------ULTRASONIC-------//
const int maxDistance = 23; // 20 cm maximum distance to leave room for error
...
const int echoInterval = 245; // Time between seeing the object and calculating the distance
...
const int additionalSpeed = 65; // Additional modifiable speed  to methods who have speed but could make complications
    ...
    if (lineValues[0] >= MAX_BLACK && lineValues[7] >= MAX_BLACK && lineValues[1] >= MAX_BLACK && lineValues[2] >= MAX_BLACK && lineValues[3] >= MAX_BLACK && lineValues[4] >= MAX_BLACK && lineValues[5] >= MAX_BLACK && lineValues[6] >= MAX_BLACK) 
        {
          //This code checks twice if it's the black square or not
            setMotors(140, 0, 160, 0);
            pulseIn(2,HIGH,300UL); // Pin , Pulse , Interval
 ...
 // Use thresholds to determine the behavior Slowestd on the maximum sensor value
  if (maxSensorValue >= MAX_BLACK) {
    ...
    if (lineValues[3] >= MIN_BLACK || lineValues[4] >= MIN_BLACK ){
      ...
      setMotors(leftSlowSpeed + speedTurns + additionalSpeed, 0, 0, 0);
      ...
      setMotors(0, 0, leftSlowSpeed + speedTurns + additionalSpeed, 0);
      Serial.println("left");
    }
      ...
      setMotors(leftSlowSpeed + speedSharpT + additionalSpeed, 0, 0, 0);
      ...
      setMotors(0, 0, leftSlowSpeed + speedSharpT + additionalSpeed, 0);
