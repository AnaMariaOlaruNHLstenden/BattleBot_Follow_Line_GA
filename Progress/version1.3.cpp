...
//-------MILLIS-------//
 ...
 unsigned long currentMillis; // Counting time unsing unsigned long to store the huge about of time that is counted.
...
const int rightSlowSpeed = 123; // Slowest speed right
...
//TODO:
// GRAB OBJECT FOR THE WHOLE WORKING TIME
// FINISH/START PART
...
void defaultLineSensor() {
  ...
  // }
  // Serial.println();
if (previoustimeLS_1 < currentMillis)
{
 for (int i = 0; i < numberOfSensors; i++) {
...
previoustimeLS_1 = 20UL + currentMillis;
 ...
 // Use thresholds to determine the behavior Slowestd on the maximum sensor value
  if (maxSensorValue >= MAX_BLACK) {
    ...
    if (lineValues[3] >= MIN_BLACK || lineValues[4] >= MIN_BLACK ){
      driveForward(leftSlowSpeed + speedOneWay, rightSlowSpeed + speedOneWay); //We start with slowest and then modify the speed to a decent speed
      Serial.print("forward");
      Serial.print(" ");
    }
    else if ( lineValues[2] >= MAX_BLACK){
      driveRight(leftSlowSpeed + speedTurns,rightSlowSpeed + speedTurns);
      Serial.print("right1111");
      Serial.print(" ");
    }
    else if (lineValues[5] >= MAX_BLACK){
      driveLeft(leftSlowSpeed + speedTurns,rightSlowSpeed + speedTurns);
      Serial.print("left111");
      Serial.print(" ");
    }
    else if (lineValues[1] >= MIN_BLACK){
      driveRight(leftSlowSpeed + speedSharpT,rightSlowSpeed + speedSharpT);
      Serial.print("right222");
      Serial.print(" ");
    }
    else if (lineValues[6] >= MIN_BLACK){
      driveLeft(leftSlowSpeed + speedSharpT,rightSlowSpeed + speedSharpT);
...
void gripToggle(){
 static unsigned long timer;
  ...
  if (currentMillis > timer) {
    ...
    timer = currentMillis + gripperToggle;
  }
}
...
void servo(int pulse) {
 static unsigned long timer;
  ...
  if (currentMillis > timer) {
    ...
    timer = currentMillis + servoInterval;
  }
}